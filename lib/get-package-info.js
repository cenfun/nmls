...
const getModule = (item, mName, type) => {
    if (item.nodeModules) {
    ...
    if (item.parent) {
        const m = getModule(item.parent, mName, type);
        if (m) {
            return m;
        }
    }
    ...
    const ignoreList = ['fsevents'];
    if (ignoreList.includes(mName)) {
        return;
    }
    Util.log(`not found module: ${mName}`);
};
...
const generateFlatMap = (item, map) => {
    Util.forEachDependencies(item, function(keys, type) {
        keys.forEach((k) => {
            const m = getModule(item, k, type);
            if (!m) {
                return;
            }
            if (map[m.path]) {
...
const generateSubsInfo = (item) => {
    ...
    item.subs.forEach((sub) => {
        ...
        if (m.optionalDependencies) {
            sub.optionalDependencies = m.optionalDependencies;
        }
