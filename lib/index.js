...
class NMLS {
    ...
    async start(option = {}) {
        ...
        if (option.workspace) {
            //const packages = await this.getPackages(projectJson.workspaces);
    ...
    async getPackages(workspaces) {
        if (!workspaces) {
            return;
        }
        const list = Util.toList(workspaces).filter(item => item);
        
        let matchedList = [];
        for (const item of list) {
            const dirs = await this.globDirs(item);
            if (Util.isList(dirs)) {
        ...
        packages = packages.filter(item => {
            const json = this.getModuleJson(item.path);
            if (!json) {
                return false;
            }
            item.json = json;
