...
class NMLS {
    ...
    initDependencies(dependencies) {
        ...
        for (var name in dependencies) {
            ...
            if (info) {
                ...
                this.initSubDependencies(item, dInfo, dependencies);

                item.size = dInfo.size;
                item.files = dInfo.files;
    ...
    initSubDependencies(parent, dInfo, from) {

        var dependencies = parent.dependencies;
        if (!dependencies) {

            var dFrom = from[parent.name];
            if (!dFrom) {
                return;
            }

            dependencies = dFrom.dependencies;
            if (!dependencies) {
                return;
            }
        ...
        for (var name in dependencies) {
            ...
            dInfo.used[name] = true;

            //add to parent
            dInfo.size += info.size;
            dInfo.files += info.files;

            //check sub dependencies
            var item = dependencies[name];
            item.name = name;
            this.initSubDependencies(item, dInfo, from);
