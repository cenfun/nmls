...
class NMLS {
    ...
    findLocalPath(parentInfo, subName) {

        var localDownPath = parentInfo.path + "/node_modules/" + subName;
        console.log(localDownPath);
        if (fs.existsSync(localDownPath)) {
            return localDownPath;
        }

        var localUpPath = "";
        var i = 0;
        while (parentInfo.parent && i < 5) {
            localUpPath = parentInfo.parent.path + "/node_modules/" + subName;
            console.log(localUpPath);
    ...
    generateSubDependencies(parentInfo, used) {
        ...
        for (var subName in dependencies) {
            ...
            var localPath = this.findLocalPath(parentInfo, subName);
            if (localPath) {
            ...
            if (!subInfo) {
                console.log("[nmls] WARN: Not found sub module: " + CGS.yellow(subName) + " which depends by " + parentInfo.path);
                continue;
            }
