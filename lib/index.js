...
class NMLS {
    ...
    async start(option = {}) {
        ...
        const moduleList = Util.getSpecifiedList(this.option.module);
        if (moduleList) {
            Util.output(`[nmls] filter modules with: ${this.option.module}`);
        }

        //generate file info
        for (const item of list) {
            getPackageInfo(item, moduleList);
        }

        const packageList = Util.getSpecifiedList(this.option.workspace);
        if (packageList) {
            Util.output(`[nmls] filter packages with: ${this.option.workspace}`);
            ...
            if (!list.length) {
                Util.output(EC.red(`[nmls] not found packages with: ${this.option.workspace}`));
                return projectInfo;
            }
        }
