...
const Util = {
    ...
    getSpecifiedModuleList: (moduleListStr) => {
        if (!moduleListStr) {
            return;
        }
    ...
    initProjectSubs: (projectInfo, moduleListStr) => {
        const subs = [];
        const moduleList = Util.getSpecifiedModuleList(moduleListStr);
        if (moduleList) {
    ...
    getModule: (item, mName) => {
        ...
        if (item.parent) {
            return Util.getModule(item.parent, mName);
        }
    },
    ...
    generateSubsInfo: (item) => {
        ...
        item.subs.forEach(sub => {
            if (sub.isGroup) {
                Util.generateSubsInfo(sub);
                return;
            }
            const m = Util.getModule(item, sub.name);
            if (!m) {
