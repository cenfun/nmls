...
const Util = {
    ...
    generateFolderFileList: (p, isIgnore = () => false) => {
        ...
        for (const name of list) {
            const absPath = path.resolve(absP, name);
            ...
            if (info.isDirectory()) {
                ...
                Util.log(EC.red(`[nmls] Unknown file: ${relPath}`));
            }
        }
        return fileList;
    },
    ...
    initModuleJson(item = {}, json = {}) {
        const keys = ['name', 'version', 'dependencies', 'devDependencies', 'optionalDependencies'];
        keys.forEach((k) => {
            const v = json[k];
            if (v) {
                item[k] = v;
    ...
    readdir(p) {
        return new Promise((resolve) => {
            fs.readdir(p, (err, list) => {
                if (err) {
                    Util.log(`[nmls] ERROR: fs.readdir: ${EC.yellow(p)}`);
                    resolve([]);
                    return;
                }
                resolve(list);
